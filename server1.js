...
io.on('connection', function(socket) {
            ...
            for(var i=0;i<queue.length;i++){
                if(queue[i]==0){
                    if(i%2==1){
                        ...
                        scacchiera[room]=[[T0N=new Torre(0,0,"N"), C0N=new Cavallo(0,1,"N"), A0N=new Alfiere(0,2,"N"), RN=new Regina(0,3,"N"), KN=new Re(0,4,"N"), A1N=new Alfiere(0,5,"N"), C1N=new Cavallo(0,6,"N"), T1N=new Torre(0,7,"N")],
    ...
    socket.on('on_move',function(data){
        ...
        let muovi=function(a){
            if(!mossa_consentita)return;
                ...
                for(i=0;i<8;i++){
                    cont=0;
                    for(j=0;j<8;j++){
                        color=scacchiera[room][i][j].prendi_colore();
                        tip=scacchiera[room][i][j].prendi_tipo();
        ...
        if(scacchiera[room][x_in][y_in].possible_move(x_out,y_out,scacchiera,room)){
            var colore=scacchiera[room][x_in][y_in].prendi_colore();            
            if(colore=="B"){
                make_move=1;
                wait_move=0;
            }
            ...
            if(mossa_consentita){
                ...
                //MOSSA CONSENTITA MA NON E' SCACCO
                    ...
                    //VEDIAMO SE E' STALLO
                    if(noone_can_move(array,wait_move,scacchiera,room)){
                        stallo=true;
                        mossa.then(a=>muovi(a));
                    }
                    else mossa.then(a=>muovi(a));
                ...
                scacchiera[room][x_out][y_out].pezzoMoveOut(x_in,y_in,x_out,y_out,temp,scacchiera,room);
                temp.set_vivo();
                mossa.then(a=>muovi(a));
            }            
        }
...
class Pezzo{
    ...
    sottoScaccoOnChange(pezzo_da_muovere,temp,pezzo_da_controllare,i,j,scacchiera,room){
        temp_x=pezzo_da_muovere.x;
        temp_y=pezzo_da_muovere.y;
        this.pezzoMoveOn(pezzo_da_muovere.x,pezzo_da_muovere.y,i,j,scacchiera,room);
        if(pezzo_da_controllare.sottoscacco.x==-1){
            this.pezzoMoveOut(temp_x,temp_y,i,j,temp,scacchiera,room);
            return true;
        }
        else pezzoMoveOut(temp_x,temp_y,i,j,temp,scacchiera,room);
        return false
    }
    pezzoMoveOn(x_in,y_in,x_out,y_out,scacchiera,room){
        scacchiera[room][x_out][y_out]=scacchiera[room][x_in][y_in];
        scacchiera[room][x_in][y_in]=new Pezzo(x_in,y_in,"O");
        scacchiera[room][x_out][y_out].modifica_xy(x_out,y_out);
    }
    pezzoMoveOut(x_in,y_in,x_out,y_out,temp,scacchiera,room){
        scacchiera[room][x_out][y_out].modifica_xy(x_in,y_in);
        scacchiera[room][x_in][y_in]=scacchiera[room][x_out][y_out];
        scacchiera[room][x_out][y_out]=temp;
    }
...
class Pedone extends Pezzo{
    ...
    protegge(x_fine,y_fine,scacchiera,room){
    ...
    possible_move(x_fine,y_fine,scacchiera,room){
        if(this.is_morto()) return false
        ...
        else if((this.colore=="B" && (this.x-x_fine==1 || (this.x-x_fine==2 && this.x==6 &&scacchiera[room][x_fine][y_fine].prendi_colore()=="O")))||((this.colore=="N")&& (this.x-x_fine==-1 || (this.x-x_fine==-2 && this.x==1 && scacchiera[room][x_fine][y_fine].prendi_colore()=="O")))){
            if((this.y-y_fine==0 && scacchiera[room][x_fine][y_fine].prendi_colore()=="O")||((this.y-y_fine)**2==1 && scacchiera[room][x_fine][y_fine].prendi_colore()==this.opposite_colore()))return true;
        }
        else return false;
    }
    can_move(scacchiera,room){
        if(this.is_morto())return false;
            ...
            if(scacchiera[room][this.x+1][this.y].prendi_colore()=="O")return true;
            if(this.y-1>=0 && scacchiera[room][this.x+1][this.y-1].prendi_colore()=="N")return true;
            if(this.y+1<8 && scacchiera[room][this.x+1][this.y+1].prendi_colore()=="N")return true;
        }
        else if(this.colore=="N" && this.x-1>=0){
            if(scacchiera[room][this.x-1][this.y].prendi_colore()=="O")return true;
            if(this.y-1>=0 && scacchiera[room][this.x-1][this.y-1].prendi_colore()=="B")return true;
            if(this.y+1<8 && scacchiera[room][this.x-1][this.y+1].prendi_colore()=="B")return true;
        }
        else return false
    }
}
class Cavallo extends Pezzo{
    ...
    possible_move(x_fine,y_fine,scacchiera,room){
        if(this.is_morto()) return false
        //Basta che si muove ad L e che non ci sia un pezzo dello stesso colore alla fine
        else if((((this.x-x_fine)**2==4 && (this.y-y_fine)**2==1)||((this.y-y_fine)**2==4 && (this.x-x_fine)**2==1))&&scacchiera[room][x_fine][y_fine].prendi_colore()!=this.colore)
        return true;
        else return false;
    }
    can_move(scacchiera,room){
        if(this.is_morto()) return false
        //Basta che si muove ad L e che non ci sia un pezzo dello stesso colore alla fine
        else if(this.x-1>=0 && this.y+2<8 && scacchiera[room][this.x-1][this.y+2].prendi_colore()!=this.colore)return true;
        else if(this.x-2>=0 && this.y+1<8 && scacchiera[room][this.x-2][this.y+1].prendi_colore()!=this.colore)return true;
        else if(this.x-1>=0 && this.y-2>=0 && scacchiera[room][this.x-1][this.y-2].prendi_colore()!=this.colore)return true;
        else if(this.x-2>=0 && this.y-1>=0 && scacchiera[room][this.x-2][this.y-1].prendi_colore()!=this.colore)return true;
        else if(this.x+1<8 && this.y-2>=0 && scacchiera[room][this.x+1][this.y-2].prendi_colore()!=this.colore)return true;
        else if(this.x+2<8 && this.y-1>=0 && scacchiera[room][this.x+2][this.y-1].prendi_colore()!=this.colore)return true;
        else if(this.x+1<8 && this.y+2<8 && scacchiera[room][this.x+1][this.y+2].prendi_colore()!=this.colore)return true;
        else if(this.x+2<8 && this.y+1<8 && scacchiera[room][this.x+2][this.y+1].prendi_colore()!=this.colore)return true;
        else return false;
    }
}
...
class Alfiere extends Pezzo{
    ...
    protegge(x_fine,y_fine,scacchiera,room){
        ...
        //x e y decrementano o aumentano con lo stesso valore in modulo a patto che non ci siano altri pezzi lungo il percorso
            ...
            return this.no_pezzi(x_fine,y_fine,scacchiera,room)
        }
        else return false;
    }
    possible_move(x_fine,y_fine,scacchiera,room){
        if(this.is_morto()) return false
        //x e y decrementano o aumentano con lo stesso valore in modulo a patto che non ci siano altri pezzi lungo il percorso
        else if(((this.x-x_fine)**2==(this.y-y_fine)**2)&& scacchiera[room][x_fine][y_fine].prendi_colore()!=this.colore){
            if(x_fine>this.x)x_fine--;
            else x_fine++;
            if(y_fine>this.y)y_fine--;
            else y_fine++;
    ...
    no_pezzi(x_fine,y_fine,scacchiera,room){
        if(x_fine==this.x && y_fine==this.y)return true;
        else if(scacchiera[room][x_fine][y_fine].prendi_colore()!="O") return false;
        else{
            if(x_fine>this.x)x_fine--;
            else x_fine++;
            if(y_fine>this.y)y_fine--;
...
class Torre extends Pezzo{
    ...
    protegge(x_fine,y_fine,scacchiera,room){
        ...
        else if(scacchiera[room][x_fine][y_fine].prendi_colore()==this.colore){
            if(x_fine==this.x){
                if(y_fine>this.y)return this.no_pezzi(x_fine,y_fine-1,scacchiera,room);
                else return this.no_pezzi(x_fine,y_fine+1,scacchiera,room);
            }
            else if(y_fine==this.y){
                if(x_fine>this.x)return this.no_pezzi(x_fine-1,y_fine,scacchiera,room);
                else return this.no_pezzi(x_fine+1,y_fine,scacchiera,room);
            }
        }
        else return false;
    }
...
class Regina extends Pezzo{
    ...
    protegge(x_fine,y_fine,scacchiera,room){
        ...
        else if((((this.x-x_fine)**2==(this.y-y_fine)**2) || (this.x==x_fine || this.y==y_fine))&& scacchiera[room][x_fine][y_fine].prendi_colore()==this.colore && this.no_pezzi(x_fine,y_fine,scacchiera,room))
        return true;
        else return false;
    }
    possible_move(x_fine,y_fine,scacchiera,room){
    ...
    no_pezzi(x_fine,y_fine,scacchiera,room){
        //COME TORRE
        if(x_fine==this.x){
            if(y_fine>this.y)return(this.no_pezzi_torre(x_fine,y_fine-1,scacchiera,room));
            else return(this.no_pezzi_torre(x_fine,y_fine+1,scacchiera,room));
        }
        if(y_fine==this.y){
            if(x_fine>this.x)return(this.no_pezzi_torre(x_fine-1,y_fine,scacchiera,room));
            else return(this.no_pezzi_torre(x_fine+1,y_fine,scacchiera,room));
        }
        // COME ALFIERE
        else {
            if(x_fine>this.x)x_fine--;
            ...
            return this.no_pezzi_alfiere(x_fine,y_fine,scacchiera,room);
        }
    }

    no_pezzi_torre(x_fine,y_fine,scacchiera,room){
        if(x_fine==this.x && y_fine==this.y)return true;
        else if(scacchiera[room][x_fine][y_fine].prendi_colore()!="O") return false;
        else{
            if(x_fine==this.x){
            ...
            return this.no_pezzi_torre(x_fine,y_fine,scacchiera,room)
        }
    }
    no_pezzi_alfiere(x_fine,y_fine,scacchiera,room){
        if(x_fine==this.x && y_fine==this.y)return true;
        else if(scacchiera[room][x_fine][y_fine].prendi_colore()!="O") return false;
        else{
            if(x_fine>this.x)x_fine--;
...
class Re extends Pezzo{
    ...
    possible_move(x_fine,y_fine,scacchiera,room){
        if(x_fine>7 || x_fine<0 || y_fine>7 || y_fine<0)return false
        //Si muove in un raggio unitario a patto che non ci siano pezzi dello stesso colore
        else if(((x_fine-this.x==0 || x_fine-this.x==1 || x_fine-this.x==-1)&&(y_fine-this.y==0 || y_fine-this.y==1 || y_fine-this.y==-1)) && scacchiera[room][x_fine][y_fine].prendi_colore()!=this.colore)
        return true;
